...
Node* newNode(string payload) {
    Node* temp = new Node;
    temp->payload = payload; //assign values
    temp->next = nullptr; //make sure the next of the new node is a nullptr
    return temp;
}
...
Node* loadGame(int n, vector<string> names) {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        if (head == nullptr) {
            ...
            prev = prev->next; //assign new node to prev
        }
    }
...
Node* runGame(Node* start, int k) { // josephus w circular list, k = num skips
    ...
    while (curr->next != curr) { // exit condition, last person standing
        ...
        delete curr;
        curr = prev->next; //assign curr to the next node
    }
