...
class BSTree {
   ...
   private:
      void insert(Node* root, const string& newString);
      bool search(Node* temp, const string& key) const;
      int height(Node* temp) const;
      void remove(Node* &temp, const string& key);

      string largest(Node* node) const;
      string smallest(Node* node) const;
